...
public class Solution54 {
    ...
    public static void main(String[] args) {
        ...
        Solution54Ⅰ solution54Ⅰ = new Solution54Ⅰ();
        System.out.println(solution54Ⅰ.spiralOrder(new int[][]{{1, 2, 3}, {4, 5, 6}, {7, 8, 9}}));
    }
...
class Solution54Ⅰ{
    ...
    public List<Integer> spiralOrder(int[][] matrix){
        List<Integer> result=new ArrayList<>();
        int m=matrix.length,n=matrix[0].length;
        int row=0,col=0,index=0,flag=0;
        boolean[][] visited = new boolean[m][n];
        while (index<m*n){
            result.add(matrix[row][col]);
            visited[row][col]=true;
            int curRow=row+directions[flag][0];
            int curCol=col+directions[flag][1];
            if(curRow<0||curRow>=m||curCol<0||curCol>=n||visited[curRow][curCol])
                flag=(flag+1)%4;
            row+=directions[flag][0];
            col+=directions[flag][1];
            index++;
        }
        return result;
    }
