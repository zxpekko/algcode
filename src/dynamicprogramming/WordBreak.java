...
public class WordBreak {
    public static void main(String[] args) {
        ...
        WordBreakⅡ wordBreakⅡ = new WordBreakⅡ();
        System.out.println(wordBreakⅡ.wordBreak("applepenapple", stringList));
    }
    ...
    public boolean wordBreak(String s, List<String> wordDict){
        ...
        for(int i=0;i< dp.length;i++){
            for(int j=0;j<wordDict.size();j++){
                if(!dp[i]&&i>=wordDict.get(j).length())
                    dp[i]=dp[i-wordDict.get(j).length()]&&judgeCanAdd(s,i-wordDict.get(j).length(),wordDict.get(j),i-1);
            }
        }
        return dp[s.length()];
    }
    public boolean judgeCanAdd(String s,int start,String subStr,int end){
        for(int i=start,j=0;i<=end;i++,j++){
