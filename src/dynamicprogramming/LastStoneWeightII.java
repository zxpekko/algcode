...
public class LastStoneWeightII {
    public static void main(String[] args) {
        ...
        LastStoneWeightIIⅡ lastStoneWeightIIⅡ = new LastStoneWeightIIⅡ();
        System.out.println(lastStoneWeightIIⅡ.lastStoneWeightII(new int[]{31, 26, 33, 21, 40}));
        System.out.println(lastStoneWeightIIⅡ.lastStoneWeightIIⅡ(new int[]{31, 26, 33, 21, 40}));
    }
...
class LastStoneWeightIIⅡ{
    public int lastStoneWeightII(int[] stones){
        ...
        return Math.abs(sum-2*dp[n-1][divide]);
    }
    public int lastStoneWeightIIⅡ(int[] stones){
        int n= stones.length;
        int sum=0;
        for(int i=0;i<n;i++)
            sum+=stones[i];
        ...
        for(int i=0;i<n;i++){
            for(int j=volumn;j>=stones[i];j--){
                dp[j]=Math.max(dp[j],stones[i]+dp[j-stones[i]]);
            }
        }
        return sum-2*dp[volumn];
    }
}